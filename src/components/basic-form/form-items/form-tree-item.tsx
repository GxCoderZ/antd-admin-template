...
export function FormTreeItem({ treeData, value, onChange, translateTitle = true }: FormTreeItemProps) {
	...
	const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		const newExpandedKeys = flattenTreeData
			.map((item) => {
				if ((translateTitle ? t(item.title) : item.title).includes(value)) {
	...
	return (
		<>
			...
			<Tree
				...
				defaultExpandAll
				// checkStrictly
				titleRender={node => translateTitle ? t(node.title as string) : node.title as string}
				onExpand={onExpand}
