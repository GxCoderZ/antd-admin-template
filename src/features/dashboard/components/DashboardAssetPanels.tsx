...
export function DashboardAssetPanels({
	...
	const canReadAnnouncements = usePermission(
	...
	const quickEntries: QuickEntry[] = [
		{
			...
			title: t("adminShell.analysis.quickEntries.users"),
		...
		},
		{
			...
			key: "roles",
			path: "/access/roles",
			title: t("adminShell.analysis.quickEntries.roles"),
		...
		},
		{
			...
			title: t("adminShell.analysis.quickEntries.announcements"),
			visible: canReadAnnouncements,
		},
		{
			icon: <AuditOutlined aria-hidden />,
			key: "audit",
			path: "/operations/audit-logs",
			title: t("adminShell.analysis.quickEntries.audit"),
			visible: canReadLogs,
		},
		{
			...
			title: t("adminShell.analysis.quickEntries.settings"),
			...
			<Row gutter={[token.marginLG, token.marginLG]}>
				<Col lg={16} xs={24}>
					<Card
						loading={loading}
						style={{ height: "100%" }}
						title={t("adminShell.analysis.trendTitle")}
					>
						<Flex vertical>
							{(statistics?.loginTrend ?? []).map((point) => {
								const total = point.success + point.failure;
								const percent =
									total === 0 ? 0 : Math.round((point.success / total) * 100);
								return (
									<Flex
										align="center"
										gap={token.margin}
									...
									>
										...
										/>
										<Text type="secondary">
											{t("adminShell.analysis.trendValue", {
				...
				<Col lg={8} xs={24}>
					<Card
						loading={loading}
						style={{ height: "100%" }}
						title={t("adminShell.analysis.quickEntryTitle")}
					>
			...
			<Row gutter={[token.marginLG, token.marginLG]}>
				<Col lg={12} xs={24}>
					<Card
						loading={loading}
						style={{ height: "100%" }}
						title={t("adminShell.analysis.todoTitle")}
					>
						<Flex vertical>
							{(statistics?.todos ?? []).map((todo) => (
								...
								>
									<Flex gap="small" vertical>
										...
										<Text type="secondary">
											{t("adminShell.analysis.dueAt", {
												time: formatDateTime(todo.dueAt, formatPreferences),
											})}
										</Text>
									</Flex>
									<Tag
				...
				<Col lg={12} xs={24}>
					<Card
						loading={loading}
						style={{ height: "100%" }}
						title={t("adminShell.analysis.recentActivityTitle")}
					>
						<Timeline
