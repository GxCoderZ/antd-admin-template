...
- `@ant-design/pro-components`、`antd-style`：仅用于已有 Ant Design Pro 页面资产和样式能力。
- `antd-img-crop`：用于现有 Ant Design Upload 头像裁剪流程。
- `@dnd-kit/*`：用于现有页面标签横向拖拽能力。
- `dayjs`、`ua-parser-js`：用于现有日期和设备信息展示。
...
- 唯一正式源码目录是 `D:\Dev\antd-admin-template`；Codex worktree 只用于隔离开发，不是最终交付目录。
- 用户验收服务统一从正式目录运行在 `http://127.0.0.1:3003`；并行 worktree 不长期启动开发服务器，不向用户提供不同端口作为最终测试地址。
- 专项任务必须在自己的工作树提交本地 Git 并报告 SHA；主任务负责检查工作树、合并提交、解决冲突并在正式目录重新执行完整质量门槛。
- 禁止直接复制临时工作目录覆盖正式仓库；合并前后均要确认无遗漏提交和无意外未提交文件。
- 跨领域扩展、批量合并或高风险架构调整前创建有语义的本地回滚 tag；普通单页修改不滥建标签。
- 默认只提交本地 Git。推送 GitHub/GitLab、修改远端仓库、Cloudflare 部署、Webhook 通知或其它外部发布必须获得当次明确授权。
- `scripts/` 只允许服务于本前端母版的开发、检查、构建和模板初始化脚本；禁止加入 GitLab 迁移、服务器备份、数据库或生产运维脚本。
...
- 用户指定 Ant Design 或 Ant Design Pro 参考页时，实施前先核对对应页面和当前可用源码；优先复用现有官方组件，保持信息结构、间距、加载和交互，只适配 React 19、Ant Design 6、国际化、Fake API、主题与响应式，不照搬旧架构、旧 API 或后端假设。
- 已有 `@ant-design/pro-components` 能准确承载参考页面时优先复用；不得同时维护手写仿制组件和官方组件两套实现。
...
- 卡片圆角、边框、背景和阴影使用 Ant Design 默认值与 Theme Token；禁止页面自行增加重阴影、异常圆角或重复标题，标题位于卡片内外由页面类型和官方参考决定。
- Drawer、Modal 使用现有共享紧凑、标准、宽型尺寸档位；长内容 body 独立滚动、操作区固定，390px 下占满可用宽度，不在领域页面随意写超宽尺寸。
...
- 查询与重置必须复用标准 Submitter，保持统一的顺序、间距和靠右算法；领域页面不得另写按钮定位逻辑。
...
- 列显示、列顺序和密度使用现有偏好存储长期保存；日期等特殊值先序列化；接口数据仍由 TanStack Query 管理。
- 禁止默认缓存整页、选中行、Modal、Drawer、未提交编辑表单和危险确认；新增管理表格必须测试恢复、重置与关闭清理。
- 管理表格行操作不超过两个且不会造成布局溢出时默认直接展示；达到三个及以上时直接展示最常用且低风险的操作（通常为编辑；角色等领域可另有一个同等高频的主操作），其余操作统一收进 Ant Design Dropdown 的“更多”；“更多”菜单项使用 `@ant-design/icons` 对应图标。
- 主标识列和操作列不可隐藏，操作列必须始终位于最后；新增列只声明重要级别、最小宽度和是否必显，不得改变公共排序、响应式和列持久化算法。
...
- 页面标签保持 Ant Design Tabs 与现有 `@dnd-kit/*` 实现，只允许横向换位。
- 仪表盘标签固定首位，不允许关闭、拖动、参与让位或执行其它标签操作。
- 标签右键菜单与右侧工具菜单复用同一套标签操作能力，不建立平行状态或重复关闭算法。
