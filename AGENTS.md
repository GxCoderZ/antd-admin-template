...
- 顶栏头像入口保持 44px，搜索、语言、主题、通知入口保持 36×36px。顶栏入口与侧栏菜单的水波纹是项目增强，不是 Ant Design Pro 默认效果；统一使用 `usePressRipple` 和 `PressRipple.module.css`。长按保持、松开淡出、连续点击重新触发；禁止叠加旧按压底色，保留悬停、焦点、选中和禁用状态。
