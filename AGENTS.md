...
- 验证范围按行为影响确定，不按文件目录机械扩大。运行前简述“修改点 -> 责任层 -> 受影响组件/页面 -> 选定测试”；能够明确定位的局部修改禁止默认跑全项目测试。

### 页面体验巡检

- 涉及布局、主题、表格、导航、Drawer、Modal、响应式或高频交互的修改，必须运行对应页面的轻量体验巡检。
- 只巡检受影响页面及必要的直接调用页；共享组件选择有实际差异的代表页面，不因所有页面都使用壳层就遍历全站。
- 涉及响应式、断点、全局布局或触摸/鼠标切换时覆盖 1440px、768px、390px，并验证宽窄切换与恢复；局部视觉或交互修改检查相关页面的桌面和 390px，明确仅适用单端时只查该端。纯逻辑且不影响页面呈现或操作时不做截图巡检。
- 巡检必须检查页面级横向溢出、文本重叠、按钮遮挡、弹层溢出、控制台错误、请求失败和明显卡顿。
- 有自动化脚本时必须记录页面打开、视口切换、核心交互耗时，并输出 p50 / p95。
- 浏览器验收必须对应当前源码；局部功能和视觉检查可用已更新的开发服务，性能、打包及发布验收必须用当前源码的生产构建。现有 Playwright 会启动生产构建与预览，不重复预先构建；复用预览服务前必须确认产物对应当前源码。动画相关检查必须观察真实启动和结束状态，计时只覆盖目标界面的可用状态，不等待无关装饰动画。
...
- **仅文档**：运行 `git diff --check`，检查修改内容和相关链接；不运行应用测试、构建或页面巡检。
- **局部组件/单页**：修改 TS/TSX 或类型契约时运行 `pnpm run typecheck`；对修改的 JS/TS 文件运行 `pnpm exec eslint <文件路径...> --max-warnings=0`；行为修改只运行相关测试文件或用例。视觉/交互修改按上节巡检对应页面，组件测试足以覆盖的非浏览器行为不再重复 E2E。
- **共享逻辑/跨页行为**：检查直接调用链，运行责任层测试及受影响消费方的代表性回归。路由、壳层、导航、登录预览改动只选择相关 E2E 文件或用例，不自动触发全量 E2E。只有共享契约变化确实影响多个领域时才扩大范围。
- **全量门禁**：跨领域大范围重构、核心状态/权限/路由契约迁移、广泛影响运行时的依赖或构建配置变更，以及 CI/发布验收，运行下列完整检查。以实际影响为依据，不因修改了一个公共文件或提交了一次 Git 就触发全量；同一最终源码状态的完整结果可用于该次门禁，不在本地和 CI 无理由重复执行。
...
- **专项检查**：模块依赖方向或跨域引用变化运行 `check:circular-deps`；文件、导出、依赖或入口的新增/删除运行 `check:unused`；打包、资源加载、动态导入、构建配置或依赖变化运行 `build:prod`；依赖变化另运行 `pnpm audit --prod`。没有相关变化时不例行执行这些专项。
- 定向运行优先使用现有入口：`pnpm test -- --run <测试文件...>`、`pnpm run test:e2e <spec 文件...> -g <用例名称>`。不为此新增测试框架、选择器脚本或第二套流水线。
- 例：只移除语言、通知、头像的 Tooltip 时，检查 `AdminShellHeader`、`NotificationPopover` 及 `e2e/header-menu-interactions.spec.ts` 的相关用例；涉及输入模式时补 `popup-input-modes.spec.ts` 的对应场景。无需重跑字典 CRUD、表格排序或全部详情页。
- 同一源码状态已通过的检查不重复执行；后续修改只重跑受影响项。测试失败先看相关输出、截图和责任层，不以反复重跑全套取得偶然通过。发现范围外问题应记录并说明，不静默扩大修复范围。
- 交付前扫描触及范围：定时器/重试、吞错、类型逃逸、规则禁用、样式覆盖、API 越界、重复状态、旧实现、死代码；所有命中必须修复或符合“当前唯一允许的例外”。
- 没有与本次影响范围相符的新鲜验证结果、必要页面证据和干净的相关 diff，不得宣称完成。报告实际执行范围；定向通过不得表述为“全项目通过”，仅文档修改不得宣称应用行为已验证。
